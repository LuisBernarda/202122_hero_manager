...
const MarvelApiUrl = process.env.MarvelApiUrl || 'http://hero_manager_marvel:8082'
const StatisticsApiUrl = process.env.StatisticsApiUrl || 'http://hero_manager_statistics:8083'
...
const start = async(mongo) => {
    ...
    agenda.define('gather data', async(job, done) => {
        ...
        for (let series of seriesDocuments) {
            let comics = await axios.post(MarvelApiUrl + "/api/marvel/comics/" + series.id)
